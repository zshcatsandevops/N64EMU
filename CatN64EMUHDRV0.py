...
class MemoryBus:
    ...
    def load_cart(self, data: bytes) -> None:
        self.cart = bytearray(data[:CART_SIZE_MAX])
        log_bus.info(f"Cartridge loaded: {len(self.cart)} bytes")
    ...
    def read32_p(self, paddr: int) -> int:
        ...
        # Cart window (read-only)
        if CART_BASE <= paddr < CART_BASE + CART_SIZE_MAX:
            off = paddr - CART_BASE
            if 0 <= off <= len(self.cart) - 4:
                # Cart is big-endian
...
class EmulatorCore:
    ...
    def load_rom(self, path: str) -> bool:
        try:
            ...
            log_bus.info(f"Loaded ROM: {path}")
            return True
        except Exception as e:
